function [K, X,M] = ste_k(triplets,F, lambda, no_dims)
%
% modified by Vicente Malave, Nov 2015 to add supervision (metric learning)
%
%
%
...
% Delft University of Technology
    ...
    while iter < max_iter && no_incr < 5
        ...
        % replace kernel projection with metric update (see paper)
        %
        %
        %
        M=featureInvPrecision*F'*K*F*featureInvPrecision;
        % update M to be PSD instead of K - note this is much smaller!
        [V, L] = eig(M);
        V = real(V); L = real(L);
        ind = find(diag(L) > 0);
        M = V(:,ind) * L(ind, ind) * V(:,ind)';
        % K is now psd by construction
        K=F*M*F';
